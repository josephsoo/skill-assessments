...
We filter the gapminder dataset to only include data from the year 1962, then plot the log transformed CO2 emissions against gdpPerCap emissions
...
First, we filter out points in the data with incomplete points for CO2 emissions (metric tons per capita)' and gdpPercap. Next, we calculate the correlation for each year, and search for the year with the greatest correlation
```{r, warning = FALSE}
...
Visually, the data is not normally distributed. Therefore, we use the non-parametric Kruskal-Wallis test to test for a significant difference in variance between continents for Energy use.
...
### Filtering the data
Here, we filter the points past 1990, and create a density plot and boxplot to compare Europe and Asia's imports of goods and services (% of GDP)
```{r, warning = FALSE}
...
### Plotting a qqplot
We plot a qqplot for asia and europe's import of goods and services to assess normality.
```{r, warning = FALSE, echo=FALSE}
...
We plot a bar chart of the top 5 countries with the greatest difference in life expectancy at birth between 1962 and 2007.
```{r, warning = FALSE}
